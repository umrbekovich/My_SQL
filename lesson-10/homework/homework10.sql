...
select SupplierName, ProductName from Suppliers s
...
select DepartmentName, COUNT(EmployeeID) as EmployeeCount from Employees e
join Departments d
on e.DepartmentID=d.DepartmentID
...
select  s.ProductID, ProductName from Products p
left join Sales s
...
select FirstName, LastName, count(OrderID) as TotalOrder from Customers c
left join Orders o
on c.CustomerID =o.CustomerID
...
select  DepartmentID, avg(Salary) as avgsalary into Avarage_salary from Employees
group by DepartmentID

select Name, Salary
 from Employees e
 join Avarage_salary a
on e.DepartmentID=a.DepartmentID
where Salary > avgsalary

--task20

select o.OrderID, ProductID, OrderDate from orders o
 left join Payments p
on o.OrderID=p.OrderID
where YEAR(OrderDate)<2020 and ProductID is null
...
SELECT 
    e1.Name AS Employee1,
	e1.Salary as e1Salary,
    e2.Name AS Employee2,
	e2.Salary as e2Salary,
    e1.ManagerID
...
select Name as StudentName,s.StudentID from Students s
