...
select Products.ProductName, SupplierName from Products
cross join Suppliers

--task2


select Name,DepartmentName from Departments
...
select (FirstName+ ' '+LastName) as fullname, OrderID from Orders
join Customers
...
select OrderID, Price from Orders o
join Products p
...
select c.FirstName+' '+ c.LastName as fullname, ProductID, SaleAmount from Customers c
join Sales s
...
select Name, CourseName from Courses c
join Enrollments e
...
select o.OrderID, Amount, TotalAmount from Orders o
join Payments p
...
select SaleID, Country from Sales s
join Customers c
...
select OrderID,Country, TotalAmount from Orders o
join Customers c
...
select name as name1, Name as name2 from Employees e
 join Departments d
 on e.DepartmentID<d.DepartmentID
...
select py.Amount, py.PaymentDate, py.PaymentMethod from Payments py
join Orders o
...
select Name, e.StudentID from Students s
...
select
     EmployeeT.Name as EmployeeName,
	 EmployeeT.Salary as EmployeeSalary,
	 ManagerT.Name as ManagerName,
	 ManagerT.Salary as ManagerSalary
from Employees as ManagerT
join Employees as EmployeeT
on ManagerT.EmployeeID=EmployeeT.ManagerID
where ManagerT.Salary<=EmployeeT.Salary


--task25


select FirstName + ' '+ LastName as Fullname, PaymentID from Customers c
join Orders o
