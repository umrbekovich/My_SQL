...
select DepartmentName, avg(Salary) as avarage_salary, COUNT(EmployeeID) as empQuantity from Employees
group by DepartmentName
...
select distinct CustomerID, sum(Quantity) from Orders
group by CustomerID
...
select DepartmentName, sum (salary) as tolat_salary, avg(Salary) as avg_salary from Employees
group by DepartmentName
having avg(Salary)>65000

--task23

select  CustomerID, COUNT(TotalAmount), MIN(TotalAmount) from Orders
group by CustomerID 
