...
intersect
select * from Products_Discounted  -- This query shows the intersections of two tables on common columns.


--We can select the corresponding columns in both tables separately to find the intersections on each corresponding column. Like this:
...
intersect
select StockQuantity from Products_Discounted 
...
select iif (Price> 1000, 'Expencive', 'Affordable') as PriceStatus from Products
...
--task14
  ...
  alter table Orders
  add  DiscountPercentage varchar(10)
  update Orders
  ...
  select CustomerID, Quantity, DiscountPercentage from Orders
