...
select top 1 CONCAT_WS(' ', EMPLOYEE_ID,FIRST_NAME, LAST_NAME) from Employees
...
select FIRST_NAME, len(FIRST_NAME) as length_of_FirsName from Employees
...
WHERE DeliveryState = 'tx'
  ...
  select CONCAT_WS(' ', String, String) from DMLTable
 ...
 /* The total number of employees in each department and the percentage of those employees who have been with the company for more than 3 years(Employees)*/
  ...
  select DEPARTMENT_ID, COUNT(EMPLOYEE_ID) as TotalEmp, () from Employees
...
select case when 'tf56sd#%OqH'
select UPPER('tf56sd#%OqH')
