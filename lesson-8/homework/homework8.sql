create database sql_class_home_8
go 
use sql_class_home_8
...
select Category, COUNT(ProductID) ProductCount from Products
group by Category
...
case when StockQuantity < 30 then 'Low Stock'
 ...
 select Country, COUNT(CustomerID) as CustomerNumbe from Customers
 group by Country
...
-- 2️⃣ Pivot Year values into separate columns
PIVOT
(
    SUM(Population)
...
select ProductID, sum(SaleAmount) as totalamount from Sales
group by ProductID
...
unpivot
(population for [year] in ([2012], [2013])) as city_population2

--task21

select Products.ProductName, COUNT(Sales.ProductID) as times_sold from Products
inner join Sales
on Products.ProductID = sales.ProductID
group by Products.ProductName

--task22
select district_name, 2012, 2013 from Population_Each_City
unpivot
(
[2012] for district_name in  (Bektemir, Chilonzor, Yakkasaroy)
) as first_table
