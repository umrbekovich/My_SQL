...
select *, len(texts)-len(REPLACE(texts, ' ', '')) SpaceCount from CountSpaces

--task6

select * from Employee

select e1.Name EmpName, e1.Salary EmpSalary, e2.Name MngName, e2.Salary MngSalary from Employee e1
join Employee e2
...
select Employee_ID, First_Name, Last_Name, Hire_Date, DATEDIFF(year, HIRE_DATE, getdate()) Years_of_service from Employees
...
group by player_id --first way using aggregate function

select a1.player_id, a1.event_date  from Activity a1
left join Activity a2
on a1.player_id=a2.player_id
and a1.event_date > a2.event_date
where a2.player_id IS NULL -- second way using self-join

--task3

select * from fruits

select left((select  SUBSTRING(fruit_list, CHARINDEX(',', fruit_list, CHARINDEX(',', fruit_list )+1)+1, len(fruit_list)) from fruits),
               CHARINDEX(',',(select SUBSTRING(fruit_list, CHARINDEX(',', fruit_list, CHARINDEX(',', fruit_list )+1)+1, len(fruit_list))from fruits))-1) from fruits

--task4

--If the employee has worked for less than 1 year → 
--If the employee has worked for 1 to 5 years → 
--If the employee has worked for 5 to 10 years → 
--If the employee has worked for 10 to 20 years → 
--If the employee has worked for more than 20 years → (Employees)
...
select Employee_ID,
	   ...
	   Last_Name, 
	   Hire_Date, 
	   DATEDIFF(year, HIRE_DATE, getdate()) Years_of_service,
	   case 
...
SELECT 
    ...
    ) AS swapped_string from MultipleVals
	
--task2

DECLARE @str NVARCHAR(100) = 'sdgfhsdgfhs@121313131';

-- Generate a tally (numbers) table on the fly
WITH Numbers AS (
    SELECT TOP (LEN(@str))
...
INTO CharactersTable   --  creates a new table
FROM Numbers;
