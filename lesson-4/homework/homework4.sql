...
SELECT TOP (5) [ID]
      ...
      ,[Gander]
      ,[HireDate]
  FROM [nodirbek06].[dbo].[Employees]

  --task2

  SELECT DISTINCT Category
FROM [f44_sql_class].[dbo].[Products];
...
select * from Employees
where age between 30 and 40 or Departmentname = 'Markeing'
...
select * from products
where price <=1000 or stockQuantity > 50
...
FROM Employees; --To avoid NULLs breaking concatenation, isnull is used

 
 /*second way*/
 SELECT CONCAT(FirstName, ' ', LastName) AS FullName
FROM Employees; -- no need to isnull
